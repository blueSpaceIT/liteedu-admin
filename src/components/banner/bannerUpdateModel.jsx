...
import { useUpdateBannerMutation } from "../../redux/features/api/banner/bannerApi";

const BannerUpdateModal = ({ banner, isOpen, onClose }) => {
  ...
  useEffect(() => {
    if (banner) {
      setCoverPhotoFile(banner?.image || null);
      setFormData({
        position: Number(banner?.position) || 0,
        viewLink: banner?.viewLink || "",
        image: banner?.image || "",
  ...
  const handleChange = async (e) => {
    ...
    // Convert position to number explicitly
    setFormData((prev) => ({
      ...prev,
  ...
  const handleSubmit = async (e) => {
    handleSubmitForm({
      e,
      apiCall: updateBanner,
      data: formData,
      params: { _id: banner?._id },
      onSuccess: () => {
        setFormData({ position: 0, viewLink: "", image: "", status: "Published" });
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="max-h-[90vh] w-[90%] max-w-2xl overflow-y-auto rounded-lg bg-white p-6 shadow-xl dark:bg-gray-900">
        <h2 className="mb-4 text-xl font-bold text-gray-900 dark:text-white">Update Banner</h2>
