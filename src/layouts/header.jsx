...
const Header = ({ collapsed, setCollapsed, userInfo }) => {
  ...
  const [localUser, setLocalUser] = useState(
    savedUser ? JSON.parse(savedUser) : userInfo || null
  );
  ...
  const matchedAdmin = adminResponse?.data?.find(
    (admin) => admin.userId?._id === localUser?._id
  );
  ...
  return (
    <header
      className={cn(
        ...
        "bg-white px-3 md:px-4 shadow-sm border-b border-slate-200",
        "transition-colors dark:bg-slate-900 dark:border-slate-800"
      )}
    >
      {/* Left: collapse + logo */}
      <div className="flex items-center gap-2 md:gap-3">
        ...
        <Link to="/" className="flex items-center gap-2 md:gap-3">
          ...
          />
          <span className="hidden text-sm font-semibold text-slate-800 md:inline dark:text-slate-100">
            Admin Dashboard
          </span>
        </Link>
      </div>
